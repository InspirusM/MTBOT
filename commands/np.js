...
const { meminta } = require(`../index.js`)
exports.run = async(client, msg, args) => {
  const serverQueue = require('../index.js').queue.get(msg.guild.id);
 if(!serverQueue) return msg.channel.send({ embed: { color: 0x1D82B6, description:'There Is Nothing Playing In The Server Right Now!'}});
  ...
  .setColor('RANDOM')
  .setAuthor(`${serverQueue.songs[0].meminta.tag}`,serverQueue.songs[0].meminta.avatarURL)
